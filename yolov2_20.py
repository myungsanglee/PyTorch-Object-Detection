...
class YoloV2Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # ============================ #
        loss_x = torch.pow(((x * mask) - (tx * mask)) * ((1-x)*x*mask), 2)
        loss_y = torch.pow(((y * mask) - (ty * mask)) * ((1-y)*y*mask), 2)
        loss_w = self.mse_loss(w * mask, tw * mask)
        loss_h = self.mse_loss(h * mask, th * mask)
        box_loss = self.lambda_coord * (loss_x.sum() + loss_y.sum() + loss_w + loss_h)
