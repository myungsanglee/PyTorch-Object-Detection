...
class YoloV3LossV3(nn.Module):
    ...
    def __init__(self, num_classes, anchors, input_size):
        ...
        # self.bce_cls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([1.]))
        # self.bce_obj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([1.]))
        
        self.bce_cls = nn.BCELoss(reduction='sum')
        self.bce_obj = nn.BCELoss(reduction='sum')
        # self.mse_obj = nn.MSELoss(reduction='sum')
        
        self.positive_class_target, self.negative_class_target = smooth_BCE(0.)
    ...
    def forward(self, input, target):
        ...
        lnobj = torch.zeros(1, device=device)  # no object loss
        batch_size = input[0].size(0)
        for layer_idx, pred in enumerate(input):
            _, _, layer_h, layer_w = pred.size()
            ...
            mask, num_targets, tbox, tconf, tcls = self._encode_target(target, self.num_classes, layer_idx, self.anchors, layer_w, layer_h, self.ignore_threshold)
            if prediction.is_cuda:
                mask = mask.cuda()
                tbox = tbox.cuda()
                tconf = tconf.cuda()
            ...
            if num_targets:
                ...
                #   FOR BOX COORDINATES Loss   #
                # ============================ #
                box_iou = bbox_iou(pbox[mask==1], tbox[mask==1], CIoU=True).squeeze()
                lbox += (1.0 - box_iou).sum()
                # print(f'lbox: {lbox}')
                # print(f'box_iou: {box_iou.size()}')
            
                # ==================== #
                #   FOR OBJECT LOSS    #
                # ==================== #
                # iou = box_iou.detach().clamp(0).type(tconf.dtype)
                # tconf[mask==1] = iou
                # print(f'tconf: {tconf[mask==1].size()}')
                lobj += self.bce_obj(pconf[tconf==1], tconf[tconf==1])
            
                # ================== #
                #   FOR CLASS LOSS   #
                # ================== #
                lcls += self.bce_cls(pcls[mask==1], tcls[mask==1])
                # print(f'lcls: {lcls}\n')
            
            # obji = self.bce_obj(pconf, tconf)
            # obji = self.mse_obj(pconf, tconf)
            # lobj += obji * self.balance[layer_idx]
            lnobj += self.bce_obj(pconf[(1-tconf)==1], tconf[(1-tconf)==1])
            
        lbox *= self.lambda_box
        lobj *= self.lambda_obj
        lcls *= self.lambda_cls
        # loss = (lbox + lobj + lcls) / batch_size
        loss = (lbox + lobj + lcls + lnobj) / batch_size
    ...
    def _encode_target(self, target, num_classes, layer_idx, anchors, layer_w, layer_h, ignore_threshold):
        ...
        for b in torch.arange(batch_size):
            for t in torch.arange(target.size(1)):
                ...
                tbox[b, best_n, gj, gi] = torch.tensor([gx - gi, gy - gj, gw/scaled_anchors[best_n][0], gh/scaled_anchors[best_n][1]])
                tconf[b, best_n, gj, gi] = 1
                tconf[b, calc_iou > ignore_threshold, gj, gi] = 1
                # tcls[b, best_n, gj, gi, int(target[b, t, 4])] = 1
        ...
        num_targets = torch.where(mask==1)[0].size(0)
        
        return mask, num_targets, tbox, tconf, tcls