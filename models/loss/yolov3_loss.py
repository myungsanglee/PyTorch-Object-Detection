...
class YoloV3Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        box_loss = self.lambda_coord * (loss_x + loss_y + loss_w + loss_h) / 4
        ...
        # object_loss = self.lambda_obj * self.mse_loss(pconf[mask==1], tconf[mask==1])
        ...
        # no_object_loss = self.lambda_noobj * self.mse_loss(pconf[noobj_mask==1], (noobj_mask * 0)[noobj_mask==1])
...
class YoloV3LossV2(nn.Module):
    ...
    def forward(self, input, target):
        ...
        for layer_idx, pred in enumerate(input):
            ...
            mask, noobj_mask, tx, ty, tw, th, tconf, tcls = self.encode_target(target, self.num_classes, layer_idx, self.anchors, layer_w, layer_h, self.ignore_threshold)
            if prediction.is_cuda:
                mask = mask.cuda()
                noobj_mask = noobj_mask.cuda()
                tx = tx.cuda()
    ...
    def encode_target(self, target, num_classes, layer_idx, anchors, layer_w, layer_h, ignore_threshold):
        """YoloV3 Loss Function
